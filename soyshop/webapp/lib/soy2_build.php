...
class SOY2DAO{
	...
	function executeQuery($query, array $binds=array(), bool $keepStatement=false){
		...
		if(SOY2DAOConfig::getOption("keep_statement") !== null){
			$keepStatement = (bool)SOY2DAOConfig::getOption("keep_statement");
		}
...
class SOY2DAOFactoryImpl extends SOY2DAOFactory {
	...
	public static function buildEntityInfomation($entity){
		...
		foreach($properties as $property){
			...
			$readOnly = (bool)self::getAnnotation(self::ANNOTATION_READ_ONLY,$propertyComment);
...
class SOY2HTMLFactory extends SOY2HTMLBase{
	...
	private static function generateWebPage(string $className, string $path){
		...
		foreach($templates as $str){
			...
			$tag = $tmp[1];
			$isEnded = (bool)(strlen($tmp[2]) OR $tag[0] == "/");
			$soyId = $tmp[3];
...
class HTMLList_LoopModel extends HTMLModel{
	...
	function getStartTag(){
		...
		$func ='<?php $'.$this->getId().'_loop_visible = !(bool)'.$step.';' .
...
class PagePlugin extends PluginBase{
    ...
    function getStartTag(){
    	if($this->getAttribute("isOverWrite")){
    		$this->isOverWrite = (bool)$this->getAttribute("isOverWrite");
