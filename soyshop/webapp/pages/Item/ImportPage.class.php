...
class ImportPage extends WebPage{
	...
	function __construct() {
		...
		$this->createAdd("plugin_list", "_common.Item.PluginCSVListComponent", array(
			"list" => SOYShopPlugin::invoke("soyshop.item.csv")->getModules()
		));
	...
	function doPost(){
		...
		$attrDao = soyshop_get_hash_table_dao("item_attribute");
		...
		foreach($lines as $line){
			...
			if(strlen($item->getCode()) > 0){
				if(strlen($item->getCategoryName())){
					$categoryId = null;

					//カテゴリIDで渡されることもある　カテゴリ名が数字のみ対策の確認を追加(idとnameが共に数字で値が異なる場合)
					if((int)$item->getCategoryName() > 0){
						$idx = (count($categoryList)) ? array_search((int)$item->getCategoryName(), $categoryList) : false;
						$categoryId = (is_numeric($idx)) ? $idx : $item->getCategoryName();
					}else{
						$categoryId = $categoryLogic->import(array("name" => $item->getCategoryName()));
					}
	...
	function insert(SOYShop_Item $item){
		try{
			return soyshop_get_hash_table_dao("item")->insert($item);
		}catch(Exception $e){
			return null;
		}
	}
	...
	function deleteItem(SOYShop_Item $item){
		try{
			soyshop_get_hash_table_dao("item")->deleteByCode($item->getCode());
		}catch(Exception $e){
	...
	private function getCustomFieldList(bool $flag=false){
	 ...
	 */
	private function _getDetailPage(int $pageId){
		static $pageIds;
		if(is_null($pageIds)) $pageIds = array();
		if(is_null($pageId) || !is_numeric($pageId)) return null;
		if(isset($pageIds[$pageId])) return $pageIds[$pageId];

		//指定されているページIDが詳細ページとして存在しているか？
		try{
			$res = soyshop_get_hash_table_dao("item")->executeQuery("SELECT id FROM soyshop_page WHERE id = :id AND type = 'detail' LIMIT 1;", array(":id" => $pageId));
		}catch(Exception $e){
			$res = array();
		}
		//IDの一番小さいページを取得する
		$pageIds[$pageId] = (isset($res[0]["id"])) ? (int)$res[0]["id"] : self::_getDefaultDetailPage();
	...
	function setDetailPage(){
		$detail = soyshop_get_hash_table_dao("item")->getByType("detail");
	...
	private function _getDefaultDetailPage(){
		...
		if(is_null($pageId)){
			try{
				$res = soyshop_get_hash_table_dao("item")->executeQuery("SELECT id FROM soyshop_page WHERE type = 'detail' ORDER BY id ASC LIMIT 1;");
