...
class DetailPage extends WebPage{
	...
	private function _buildForm(int $id){
		...
		$category = (!is_null($item->getCategory()) && isset($categories[$item->getCategory()])) ? $categories[$item->getCategory()] : new SOYShop_Category();
		...
		$this->addInput("item_order_period_start", array(
			...
			"value" => (is_numeric($item->getOrderPeriodStart())) ? soyshop_convert_date_string($item->getOrderPeriodStart()) : "",
		...
		$this->addInput("item_order_period_end", array(
			...
			"value" => (is_numeric($item->getOrderPeriodEnd())) ? soyshop_convert_date_string($item->getOrderPeriodEnd()) : "",
			"id" => "order_period_end",
			"readonly" => true
		));
		...
		$this->addInput("item_open_period_start", array(
			...
			"value" => (is_numeric($item->getOpenPeriodStart())) ? soyshop_convert_date_string($item->getOpenPeriodStart()) : "",
		...
		$this->addInput("item_open_period_end", array(
			...
			"value" => (is_numeric($item->getOpenPeriodEnd())) ? soyshop_convert_date_string($item->getOpenPeriodEnd()) : "",
			"id" => "open_period_end",
			"readonly" => true
		));
	}
