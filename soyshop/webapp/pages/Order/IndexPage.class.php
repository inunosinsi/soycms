...
class IndexPage extends WebPage{
	...
	function __construct($args){
		...
		//検索を実行するか？
		$doSearch = true;
		if((int)$config->getIsDisplayWhenIsSearchCondition() === 1){
			$doSearch = (is_array($search) && count($search));
		}
		...
		define("SOYSHOP_ORDER_SEARCH_TOTAL", $total);
	...
	function getFooterMenu(){
		//検索結果がない場合はfooterメニューは表示しない
		if(SOYSHOP_ORDER_SEARCH_TOTAL === 0) return null;
