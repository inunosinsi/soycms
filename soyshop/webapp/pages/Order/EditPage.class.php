...
class EditPage extends WebPage{
		...
		if(soy2_check_token()){
			...
			if($isChange){
				...
				foreach($itemOrders as $itemOrder){
					if($taxLogic->isReducedTaxRateItem($itemOrder->getItemId())){	//軽減税率対象商品
				...
				//税金の再計算
				$module = self::_calculateConsumptionTax($price, $modulePrice, $reducedRatePrice);
	...
	private function _calculateConsumptionTax(int $price, int $modulePrice, int $reducedRateTotal){
		...
		if($config->getConsumptionTax() == SOYShop_ShopConfig::CONSUMPTION_TAX_MODE_ON){
			//外税(プラグインによる処理)
			return self::_setConsumptionTax($config, $total, $reducedRateTotal);
		}elseif($config->getConsumptionTaxInclusivePricing() == SOYShop_ShopConfig::CONSUMPTION_TAX_MODE_ON){
			//内税(標準実装)
			return self::_setConsumptionTaxInclusivePricing($config, $total);
	...
	private function buildForm(SOYShop_Order $order){
		...
		$isDeliveryTime = (strlen((string)$deliveryTimeConfigForm) > 0);
		...
		foreach(array("office", "name", "reading", "area", "address", "telephoneNumber", "zipCode") as $idx){
			switch($idx){
				...
				case "address":
					...
					break;
				case "telephoneNumber":	// 電話番号の場合、090XXXXXXXXと90XXXXXXXXが同じだと見なされるので、厳格にチェックに変更
					if(isset($addr[$idx]) && isset($newAddr[$idx]) && ((string)$addr[$idx] !== (string)$newAddr[$idx])){
