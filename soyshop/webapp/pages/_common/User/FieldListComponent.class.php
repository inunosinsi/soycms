...
class FieldListComponent extends HTMLList{
	...
	protected function populateItem($entity, $key){
		...
		$this->addLink("toggle_config", array(
			...
			"style" => ($entity->getDefaultValue() || $entity->getIsRequired() || $entity->getIsOrderDetail() || $entity->getIsOrderCsv() || $entity->getOption()) ? "background-color:yellow;" : ""
//			"visible" => $entity->hasOption()
		...
		$this->addCheckBox("is_order_detail", array(
			"name" => "config[isOrderDetail]",
			"value" => SOYShop_UserAttribute::IS_ORDER_DETAIL,
			"selected" => ($entity->getIsOrderDetail() == SOYShop_UserAttribute::IS_ORDER_DETAIL),
			"label" => "注文詳細で入力内容を出力する"
		));

		$this->addCheckBox("is_order_csv", array(
			"name" => "config[isOrderCsv]",
			"value" => SOYShop_UserAttribute::IS_ORDER_CSV,
			"selected" => ($entity->getIsOrderCsv() == SOYShop_UserAttribute::IS_ORDER_CSV),
			"label" => "注文CSV出力で項目を追加する"
		));
