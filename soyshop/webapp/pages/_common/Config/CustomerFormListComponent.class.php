...
class CustomerFormListComponent extends HTMLList{
	...
	protected function populateItem($entity,$key){
		...
		$this->addCheckBox("display_form", array(
			"selected" => (!is_null($key) && $key !== "mailAddress" && isset($this->formConfig[$key]) && is_bool($this->formConfig[$key])) ? $this->formConfig[$key] : true,
			"value" => 1,
		...
		$this->addCheckBox("form_required", array(
			"selected" => (!is_null($key) && $key !== "mailAddress" && isset($this->customerConfig[$key]) && is_bool($this->customerConfig[$key])) ? $this->customerConfig[$key] : true,
			"value" => 1,
			"onclick" => ($key == "mailAddress") ? "return false" : null,
		...
		$this->addCheckBox("admin_form", array(
			"selected" => (!is_null($key) && $key !== "mailAddress" && isset($this->adminConfig[$key]) && is_bool($this->adminConfig[$key])) ? $this->adminConfig[$key] : true,
			"value" => 1,
			"onclick" => ($key == "mailAddress") ? "return false" : null,
