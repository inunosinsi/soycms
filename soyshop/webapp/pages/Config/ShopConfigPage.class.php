...
class ShopConfigPage extends WebPage{
	...
	function doPost(){
		...
		foreach(array(
			...
			"isDisplayWhenIsSearchCondition" => 0,
			"displayStockCount" => 0,
			"ignoreStock" => 0,
	...
	function buildForm(){
		...
		$this->addCheckBox("isDisplayWhenIsSearchCondition", array(
			"name" => "Config[isDisplayWhenIsSearchCondition]",
			"value" => 1,
			"selected" => $config->getIsDisplayWhenIsSearchCondition(),
			"label" => "注文一覧で検索条件がある時のみ注文一覧を表示する"
		));
		...
		$this->addInput("birthdayYearMax", array(
			"name" => "Config[birthdayYearMax]",
			"value" => $config->getBirthdayYearMax(),
			"style" => "width:40px"
		));

		$this->addInput("birthdayYearRange", array(
			"name" => "Config[birthdayYearRange]",
			"value" => $config->getBirthdayYearRange(),
			"style" => "width:60px"
		));
		...
		$this->createAdd("placeholder_list", "_common.Config.PlaceholderListComponent", array(
			"list" => $config->getCustomerDisplayFormConfigList(),
			"placeholderConfig" => $config->getPlaceholderConfig()
		));
