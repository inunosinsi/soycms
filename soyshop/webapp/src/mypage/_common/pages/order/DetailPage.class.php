...
class DetailPage extends MainMyPagePageBase{
	...
	private function getAddressList(SOYShop_Order $order, string $mode="send"){
		...
		foreach(array("name", "reading", "office", "zipCode", "area", "address1", "address2", "address3", "address4", "tel", "telephoneNumber") as $t){
		...
		$this->addLabel($prefix . "_address_full", array(
			"text" => ($prefix == "user") ? $order->getFullAddressText() : $order->getFullClaimedAddressText()
		));
	}

	//taxモジュールが登録されているか？をチェックする
	private function checkTaxModule(array $modules){
	...
	//小計時のアイテムの総個数
	private function getSubtotalItemCount(array $itemOrders){
		if(count($itemOrders) === 0) return 0;
	...
	private function getSubtotalPrice(array $itemOrders){
		if(count($itemOrders) === 0) return 0;
