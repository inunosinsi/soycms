...
class SOYShop_UserAttribute {
	...
	//注文詳細で項目追加
	const IS_ORDER_DETAIL = 1;
	const NO_ORDER_DETAIL = 0;

	//注文CSV出力で項目追加
	const IS_ORDER_CSV = 1;
	const NO_ORDER_CSV = 0;
...
class SOYShop_UserAttributeConfig{
	...
	function hasOption(){
		return (bool)($this->getType() == SOYShop_UserAttribute::CUSTOMFIELD_TYPE_RADIO || $this->getType() == SOYShop_UserAttribute::CUSTOMFIELD_TYPE_CHECKBOX || $this->getType() == SOYShop_UserAttribute::CUSTOMFIELD_TYPE_SELECT);
	}
	function hasRadioOption(){
		return (bool)($this->getType() == SOYShop_UserAttribute::CUSTOMFIELD_TYPE_RADIO);
	}
	...
	function isIndex(){
		return (bool)$this->config["isIndex"];
	}
	...
	function getIsOrderDetail(){
		return (isset($this->config["isOrderDetail"])) ? $this->config["isOrderDetail"] : 0;
	}
	function setIsOrderDetail($isOrderDetail){
		$this->config["isOrderDetail"] = $isOrderDetail;
	}
	function getIsOrderCsv(){
		return (isset($this->config["isOrderCsv"])) ? $this->config["isOrderCsv"] : 0;
	}
	function setIsOrderCsv($isOrderCsv){
		$this->config["isOrderCsv"] = $isOrderCsv;
	}
}
