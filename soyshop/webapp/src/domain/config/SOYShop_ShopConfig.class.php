...
class SOYShop_ShopConfig {
	...
	private $isDisplayWhenIsSearchCondition = 0;	//注文一覧で検索条件がある時のみ注文一覧を表示する
	...
	private $birthdayYearMax = 10;
	private $birthdayYearRange = 100;
	...
	private $placeholderConfig = array();
	...
	function setCustomerDisplayFormConfig($array){
		...
		foreach($this->customerDisplayFormConfig as $key => $value){
			$this->customerDisplayFormConfig[$key] = (bool)@$array[$key];
		}
	...
	function setCustomerInformationConfig($array) {
		...
		foreach($this->customerInformationConfig as $key => $value){
			$this->customerInformationConfig[$key] = (bool)@$array[$key];
		}
	...
	function setCustomerAdminConfig($array) {
		...
		foreach($this->customerAdminConfig as $key => $value){
			$this->customerAdminConfig[$key] = (bool)@$array[$key];
		}
	...
	function setPlaceholderConfig($placeholderConfig){
	...
	function setSendAddressDisplayFormConfig($array){
		...
		foreach($this->sendAddressDisplayConfig as $key => $value){
			$this->sendAddressDisplayConfig[$key] = (bool)@$array[$key];
		}
	}
	...
	function setSendAddressInformationConfig($array) {
		...
		foreach($this->sendAddressInformationConfig as $key => $value){
			$this->sendAddressInformationConfig[$key] = (bool)@$array[$key];
		}
	}
	...
	function setOrderItemConfig($array){
		...
		foreach($this->orderItemConfig as $key => $value){
			$this->orderItemConfig[$key] = (bool)@$array[$key];
		}
	}
	...
	function getIsDisplayWhenIsSearchCondition(){
		return $this->isDisplayWhenIsSearchCondition;
	}
	function setIsDisplayWhenIsSearchCondition($isDisplayWhenIsSearchCondition){
		$this->isDisplayWhenIsSearchCondition = $isDisplayWhenIsSearchCondition;
	}
	...
	function getBirthdayYearMax(){
		return $this->birthdayYearMax;
	}
	function setBirthdayYearMax($birthdayYearMax){
		$this->birthdayYearMax = $birthdayYearMax;
	}

	function getBirthdayYearRange(){
		return $this->birthdayYearRange;
	}
	function setBirthdayYearRange($birthdayYearRange){
		$this->birthdayYearRange = $birthdayYearRange;
	}
