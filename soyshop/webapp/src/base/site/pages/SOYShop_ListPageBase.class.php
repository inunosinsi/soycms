...
class SOYShop_ListPageBase extends SOYShopPageBase{
	...
	function getItemsByCategory(array $args){
		...
		if(count($args) > 0){
			...
			if($category->getIsOpen() == SOYShop_Category::NO_OPEN){
				$this->error = new Exception("Don't have public permission in the specified category.");
			...
			if(!is_numeric($obj->getDefaultCategory())){	//404で返す
				$this->error = new Exception("There is no default category setting.");
				return array(array(), 0);
			}
	...
	function insertMeta(){
		...
		//descriptionの挿入
		$cnf = SOYShop_DataSets::get("common.category_navigation", array());
		if(isset($cnf[$categoryId]["description"])){
			$this->getHeadElement()->insertMeta("description", $cnf[$categoryId]["description"]);
		}
	}
