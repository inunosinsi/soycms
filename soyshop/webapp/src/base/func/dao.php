...
function soyshop_get_item_children(int $itemId, bool $isOpen=false, string $sortCondition="item_code desc"){
	...
	try{
		$GLOBALS["soyshop_item_children_hash_table"][$idx] = ($isOpen) ? $dao->getByTypeIsOpenNoDisabled($itemId) : $dao->getByTypeNoDisabled((string)$itemId);
	}catch(Exception $e){
...
/** カートに入れる事ができる商品が必ず1個の時に使用 **/
function soyshop_get_first_item_order(int $orderId){
	$itemOrders = soyshop_get_item_orders($orderId);
	if(!count($itemOrders)) return new SOYShop_ItemOrder();

	foreach($itemOrders as $itemOrderId => $itemOrder){
		return $itemOrder;
	}
