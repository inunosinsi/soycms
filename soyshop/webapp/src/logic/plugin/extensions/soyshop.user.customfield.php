...
class SOYShopUserCustomfield implements SOY2PluginAction{
	...
	/**
	 * 注文CSVでユーザーカスタムフィールドの入力内容を出力する
 	 * @return Array array(fieldId => name)
	 */
	function userCsv(){
		return array();
	}
...
class SOYShopUserCustomfieldDelegateAction implements SOY2PluginDelegateAction{
	...
	function run($extetensionId, $moduleId, SOY2PluginAction $action){
		...
		switch($this->mode){
			...
			case "order_detail":
				$_arr = $action->userDetail((int)$this->userId);
				if(is_array($_arr) && count($_arr)) $this->_list += $_arr;	
				break;
			case "order_csv":
				$_arr = $action->userCsv();
				if(is_array($_arr) && count($_arr)) $this->_list += $_arr;	
