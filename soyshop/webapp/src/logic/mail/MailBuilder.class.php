...
class MailBuilder implements SOY2LogicInterface,SOYShop_MailBuilder{
	...
	private function buildOrderInfo(SOYShop_Order $order, array $itemOrders){
		...
		foreach($itemOrders as $itemOrder){
			...
			$str .= self::printColumn(number_format($itemOrder->getItemCount())." 点");
	...
	private function buildDeliveryInfo(SOYShop_Order $order){
		...
		$mail[] = self::printColumn("住所","left",10) . $order->getFullAddressText();
	...
	private function buildUserInfoMailBody(SOYShop_Order $order, SOYShop_User $user){
		...
		$mail[] = self::printColumn("郵便番号","left",10) . $address["zipCode"];
		$mail[] = self::printColumn("住所","left",10) . $order->getFullClaimedAddressText();
