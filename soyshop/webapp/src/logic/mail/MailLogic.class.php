...
class MailLogic extends SOY2LogicBase{
	...
	function sendMail(string $sendTo="admin", string $title="", string $body="", string $sendToName="", $order=null, bool $orderFlag=false){
		...
		if(strlen($this->serverConfig->getReturnMailAddress()) > 0){
			...
			$replyToArray[] = $replyTo->getString();
		}

		// Gmail API(OAuth2.0認証)による送信
		if((int)$this->serverConfig->getIsUseGmailApiAuth() === 1){
			if(SOY2Logic::createInstance("logic.mail.SOYShopGmailLogic")->sendMail(
				$sendTo, 
				$title, 
				$body, 
				$sendToName, 
				$this->serverConfig->getAdministratorMailAddress(),
				$this->serverConfig->getAdministratorName(),
				$replyToArray
			)){
				return SOY2Logic::createInstance("logic.mail.SOYShopGmailLogic")->sendMail(
					$this->serverConfig->getAdministratorMailAddress(), 
					$title, 
					$body, 
					$sendToName, 
			...
			if(count($replyToArray) > 0) $this->send->setHeader("Reply-To", $replyToArray[0]);
