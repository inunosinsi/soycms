...
class SearchOrderLogic extends SOY2LogicBase{
	...
	public static function getInstance($className, $args){
		//import
		soyshop_get_hash_table_dao("order");
		soyshop_get_hash_table_dao("item_orders");
	 ...
	 * 検索条件からSQLを構築
	 */
	function setSearchCondition(array $cnds){
		$table = $this->getTable();
		$where = array();
		$binds = array();
	 ...
	 * マイページの購入履歴一覧で使う
	 */
	public function setSearchConditionForMyPage(int $userId, array $cnds=array()){
	 ...
	 * 備考用の検索ワードの配列を取得
	 */
	function getOrderMemoWords(string $str){
		static $array;
	...
	function setOrder($order){
		if(!is_null($order) && isset($this->sorts[$order])){
