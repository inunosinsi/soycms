...
class BulletinBoardInstall extends SOYShopPluginInstallerBase{
	...
	function onInstall(){
		...
		$cfg = SOYShop_ShopConfig::load();
		$cfg->setAppName("SOY Board");	//仮
		$cfg->setAppLogoPath(dirname(SOY2PageController::createLink("")) . "/app/css/images/main/logo.png");	//ロゴ画像の変更
		$cfg->setDisplayOrderAdminPage(0);	//注文ページを非表示
		$cfg->setDisplayItemAdminPage(0);	//商品ページを非表示
		$cfg->setAllowMailAddressLogin(1);	//メールアドレスでマイページにログインすることを禁止する
		$cfg->setAllowLoginIdLogin(2);		//ログインIDでマイページにログインすることを許可する
		$cfg->setDisplayUserOfficeItems(0);	//勤務先情報を非表示
		$cfg->setDisplayOrderButtonOnUserAdminPage(0);	//顧客詳細のページで注文ボタンを非表示
		//$cfg->setInsertDummyMailAddressOnAdminRegister(1);	//ダミーメールアドレスを許可する
		$cfg->setDisplayUserProfileItems(0);				//プロフィール関連を非表示
		$items = $cfg->getCustomerAdminConfig();
		...
		$cfg->setCustomerAdminConfig($items);
		$cfg->setCustomerDisplayFormConfig($items);

		//下記を必須項目から外す
		$items["nickname"] = false;
		$items["url"] = false;
		$cfg->setCustomerInformationConfig($items);

		SOYShop_ShopConfig::save($cfg);
		...
		$cfgs = UserCustomSearchFieldUtil::getConfig();
		if(!is_array($cfgs) || !count($cfgs)){
			try{
			...
			UserCustomSearchFieldUtil::saveConfig($cfgs);
		}
