    ...
    function convertText($format){
        ...
        switch($pageType){
            case SOYShop_Page::TYPE_LIST:
                ...
                if(isset($current)) $format = str_replace("%CATEGORY_NAME%", $current->getOpenCategoryName(), $format);
                break;
            case SOYShop_Page::TYPE_DETAIL:
                $current = $this->obj->getCurrentItem();
                if(isset($current)) $format = str_replace("%ITEM_NAME%", $current->getOpenItemName(), $format);
                break;
