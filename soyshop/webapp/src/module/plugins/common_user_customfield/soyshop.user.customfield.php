...
class CommonUserCustomfieldModule extends SOYShopUserCustomfield{
	 ...
	 */
	function userDetail(int $userId){
		self::prepare();
		if(!count($this->list)) return array();

		$_arr = array();
		foreach($this->list as $field){
			if((int)$field->getIsOrderDetail() !== SOYShop_UserAttribute::IS_ORDER_DETAIL) continue;
			$_arr[] = array(
				"name" => $field->getLabel(),
				"value" => soyshop_get_user_attribute_value($userId, $field->getFieldId())
			);
		}
		return $_arr;
	}
	...
	function userCsv(){
		...
		$_arr = array();
		foreach($this->list as $field){
			if((int)$field->getIsOrderCsv() !== SOYShop_UserAttribute::IS_ORDER_CSV) continue;
			$_arr[$field->getFieldId()] = $field->getLabel();
		}
		return $_arr;
	}
