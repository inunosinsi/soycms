...
class ShoppingMallUserSearch extends SOYShopUserSearch{
	...
	function setParameter($params){
		...
		if(count($objs)){
			...
			$queries[] = "id IN (SELECT user_id FROM soyshop_order WHERE id IN (SELECT order_id FROM soyshop_orders WHERE item_id IN (" . implode(",", $itemIds) . ")))";
...
SOYShopPlugin::extension("soyshop.user.search", "shopping_mall", "ShoppingMallUserSearch");
