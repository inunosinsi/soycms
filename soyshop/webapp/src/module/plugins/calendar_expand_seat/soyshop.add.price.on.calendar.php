...
class CalendarExpandSeatAddPriceOnCalendar extends SOYShopAddPriceOnCalendarBase{
	...
	function doPost(int $scheduleId){
		if($scheduleId > 0 && isset($_POST["ChildPrice"]) && is_numeric($_POST["ChildPrice"]) && (int)$_POST["ChildPrice"] >= 0){
			$dao = soyshop_get_hash_table_dao("schedule_price");
	...
	function list(int $scheduleId){
		try{
			$price = soyshop_get_hash_table_dao("schedule_price")->get($scheduleId, "child_price")->getPrice();
		}catch(Exception $e){
			return array();
		}
