...
class CMSApplication {
	...
	public static function printTabs(){
		...
		if($isThreeTemp){
			...
			if($isIframe && file_exists(CMS_APPLICATION_WEBAPP . APPLICATION_ID . "/iframe.php")) include_once(CMS_APPLICATION_WEBAPP . APPLICATION_ID . "/iframe.php");
			unset($iframeDir);
		}
	...
	public static function run(){
		...
		//キャッシュディレクトリの指定
		$cacheDir = CMS_APPLICATION_ROOT_DIR . "cache/".$self->applicationId."/";
		...
		//アプリケーションのチェック
		if(!file_exists(CMS_APPLICATION_WEBAPP . $self->applicationId) || !file_exists(CMS_APPLICATION_WEBAPP . $self->applicationId . "/admin.php")){
			SOY2PageController::redirect("../admin/");
			exit;
		}
		...
		//設定ファイルの読み込み
		$self->properties = (file_exists(CMS_APPLICATION_WEBAPP . APPLICATION_ID . "/application.ini")) ? parse_ini_file(CMS_APPLICATION_WEBAPP . APPLICATION_ID . "/application.ini") : array();
		...
		//アプリケーションの読み込み
		include_once(CMS_APPLICATION_WEBAPP . APPLICATION_ID . "/admin.php");
	...
	public static function display(){
		...
		if(isset($_GET["mode"]) && $_GET["mode"] == "print"){
			include_once(APPLICATION_BASE_PATH . "print.php");
		}else{
			...
			$logoDir = CMS_APPLICATION_WEBAPP . $self->applicationId . "/logo/";
			$fileName = null;
			...
			include_once(APPLICATION_BASE_PATH . $self->mode . ".php");
		}
	}
