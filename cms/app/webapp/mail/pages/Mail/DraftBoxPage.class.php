...
class DraftBoxPage extends WebPage{
    ...
    function __construct($args) {
    	...
    	//合計件数取得
    	$total = $mailDAO->countDraftMail();

    	//メール取得
    	$mailDAO->setOrder("update_date desc");
    	$mailDAO->setLimit($limit);
    	$mailDAO->setOffset($offset);
    	$mails = $mailDAO->getDraftMail();

    	//メール一覧
    	$this->createAdd("mail_list","_common.DraftBoxMailListComponent",array(
    		"list" => $mails
    	));
    	...
    	//ページャー作成
    	$pageURL = SOY2PageController::createLink("mail.Mail.DraftBox");
