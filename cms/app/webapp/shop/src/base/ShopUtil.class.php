...
class ShopUtil{
	...
	public static function tryDefaultLogin(){
		...
		if(count($siteRoles)==1){
			...
			$site = self::getSite($siteRole->getSiteId());
	 ...
	 * サイト情報を返す
	 */
	private static function getSite($siteId){
		$old = self::switchConfig();
		self::setCMSDsn();
	 ...
	 */
	public static function getSiteUrl($site){
		$siteUrl = $site->getUrl();
	 ...
	 * ショップのDBに接続する
	 */
	public static function setShopSiteDsn($site){
	 ...
	 * ショップの定数宣言
	 */
	public static function setShopSiteConfig($site){
