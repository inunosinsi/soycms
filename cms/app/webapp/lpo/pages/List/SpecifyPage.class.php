...
class SpecifyPage extends WebPage{
    ...
    function __construct() {
		...
		try{
			...
			$total = $dao->countByMode(SOYLpo_List::MODE_URL);
			$dao->setLimit($limit);
			$dao->setOffset($offset);
			$list = $dao->getByMode(SOYLpo_List::MODE_URL);
		}catch(Exception $e){
			$list = array();
		}
		...
		$pager->setPageUrl(SOY2PageController::createLink(APPLICATION_ID.".List.Specify"));
		$pager->setPage($page);
