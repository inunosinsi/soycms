...
class SOYGallery_ImageComponent extends SOYBodyComponentBase{
	...
	function execute(){
		...
		$gallery = soygallery_get_gallery_object_by_gallery_id(trim($this->getAttribute("app:gallery")));
		...
		self::_build($gallery->getGalleryId(), $gallery->getConfigArray());

		parent::execute();
	}

	private function _build(string $galleryId, array $config){
		...
		// 次のページ以降で画像がない場合はNotFound
		if($page > 1 && !count($images)) soycms_jump_notfound_page();
