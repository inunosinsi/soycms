	...
	function doPost(){
		...
		if(soy2_check_token()){
			...
			if($_POST["create"]){
				...
				if(SOY2Logic::createInstance("site_include.plugin.CustomSearchField.logic.DataBaseLogic")->addColumn($key, $_POST["custom_type"])){
					...
					CustomSearchFieldUtil::saveConfig($cnfs);
		...
		if(isset($_POST["update_advance"])){
			...
			foreach(array("option", "other", "default", "br") as $t){
				$cnfs[$key][$t] = (isset($_POST["config"][$t])) ? $_POST["config"][$t] : null;
			}
			
			$cnfs[$key]["isSearchItem"] = (isset($_POST["config"]["isSearchItem"])) ? (int)$_POST["config"]["isSearchItem"] : 0;
