<?php
function soycms_entry_calendar_after_next($html, $page){

	$obj = $page->create("entry_calendar_after_next", "HTMLTemplatePage", array(
		"arguments" => array("entry_calendar_after_next", $html)
	));
	...
	if(file_exists(_SITE_ROOT_ . "/.plugin/entry_calendar.active")){
		...
		$m = (isset($_GET["m"]) && is_numeric($_GET["m"])) ? (int)$_GET["m"] + 2 : (int)date("n") + 2;
		if($m > 12) {
			$y++;
			$m -= 12;
		}
