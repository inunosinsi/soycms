...
class SiteCreateLogic extends SOY2LogicBase{
	 ...
	 * @param $siteId サイトのID
	 */
	public function createNewSite(string $siteId, string $dbType=SOYCMS_DB_TYPE){
	 ...
	 * パスの設定
	 */
	private function prepare(string $siteId){
	 ...
	 * SOY Shop用の設定
	 */
	private  function prepareSOYShop(Site $site){
	 ...
	 * @param int $encoding
	 */
	public function initSiteConfig(string $siteName, int $encoding){
	 ...
	 */
	public function initDefaultPage(string $siteUrl){
	...
	private function initDB(string $dbType=SOYCMS_DB_TYPE){
	...
	private function &getSitePDO(string $dbType=SOYCMS_DB_TYPE){
	 ...
	 * データベースの作成
	 */
	public function createDataBase(string $siteId, string $dbType=SOYCMS_DB_TYPE){
	 ...
	 * データベース名：MySQL用
	 */
	public function getDatabaseName(string $siteId){
		return "soycms_".$siteId;
	}

	private function createHtaccess(string $siteId, bool $isRoot=false){
	...
	public function getHtaccess(string $siteId, bool $isRoot=false){
		$tmp = array();
	 ...
	 */
	private function createDenyAccessHtaccess(string $dir){
	...
	private function createController(string $siteId, string $dbType=SOYCMS_DB_TYPE){
	 ...
	 * @return String
	 */
	public function getController(string $siteId, string $dbType=SOYCMS_DB_TYPE){
		...
		return $this->__getController($this->dsn, (string)$this->user, (string)$this->pass, $dbType);
	}

	/**
	 * @return String
	 */
	public function getControllerForRenew(string $site, string $dbType=SOYCMS_DB_TYPE){
	 ...
	 * @return String
	 */
	private function __getController(string $dsn, string $dbUser="", string $dbPassword="", string $dbType=SOYCMS_DB_TYPE){
	 ...
	 */
	private function makeExecutableForCGI(string $filename){
		if(defined("SOYCMS_PHP_CGI_MODE") && SOYCMS_PHP_CGI_MODE == true){
	 ...
	 */
	private function setDefaultDBInfo(string $siteId, string $dbType=SOYCMS_DB_TYPE){
		switch($dbType){
			case "mysql":
				$this->user = ADMIN_DB_USER;
				$this->pass = ADMIN_DB_PASS;
