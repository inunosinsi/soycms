<?php
class PaymentPayJpRecurringNotification extends SOYShopNotification{
	...
	function execute(){
		...
		// @ToDo
		$logic = SOY2Logic::createInstance("module.plugins.payment_pay_jp_recurring.logic.PayJpRecurringLogic");
		$logic->initPayJp();
		list($res, $err) = $logic->retrievePlan($payJpId);

		$ok = true;

		if($res->three_d_secure_status != "verified"){
			if(PayJpRecurringUtil::isAttempt() && $res->three_d_secure_status == "attempted"){
				// throw
			}else{
				$ok = false;	
			}
...
SOYShopPlugin::extension("soyshop.notification", "payment_pay_jp_recurring", "PaymentPayJpRecurringNotification");
