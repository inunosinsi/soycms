...
class DataBaseLogic extends SOY2LogicBase{
	...
	function getByItemId(int $itemId, $lang=null){
		...
		$langId = (class_exists("UtilMultiLanguageUtil")) ? UtilMultiLanguageUtil::getLanguageId($lang) : "jp";
		try{
			$res = $dao->executeQuery("SELECT * FROM " . $this->getTableName() . " WHERE item_id = :item_id AND lang = :lang LIMIT 1", array(":item_id" => $itemId, ":lang" => $langId));
		}catch(Exception $e){
			return array();
		}
	...
	function getByItemIds(array $itemIds, $lang=null){
		...
		$langId = (class_exists("UtilMultiLanguageUtil")) ? UtilMultiLanguageUtil::getLanguageId($lang) : "jp";
		try{
			$res = $dao->executeQuery("SELECT * FROM " . $this->getTableName() . " WHERE item_id IN (".implode(",", $itemIds).") AND lang = :lang LIMIT 1", array(":lang" => $langId));
		}catch(Exception $e){
			return array();
		}
