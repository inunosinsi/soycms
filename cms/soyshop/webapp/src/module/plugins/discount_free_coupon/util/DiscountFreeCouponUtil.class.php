...
class DiscountFreeCouponUtil {
	...
	public static function getDisitsMin(){
		...
		return (isset($config["digitsMin"]) && (int)$config["digitsMin"] > 0) ? (int)$config["digitsMin"] : 4;
	}
	public static function getDisitsMax(){
		$config = self::getConfig();
		return (isset($config["digitsMax"]) && (int)$config["digitsMax"] > 0) ? (int)$config["digitsMax"] : 16;
	}
}
?>