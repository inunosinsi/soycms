...
class ImportItemInfoLogic extends ExImportLogicBase{
	...
	private $labels = array("商品ID","商品名","表示ステータス(公開・非公開)","検索ワード(コメント3)","備考欄(SHOP専用)","一覧-メイン画像","詳細-メイン画像","削除フラグ","商品コード","在庫数","在庫無制限フラグ","通常価格","販売価格","カテゴリID");
	...
	function execute(){
		...
		foreach($lines as $line){
			...
			for($i = 1; $i < count($this->factors); $i++){
				...
				switch($i){
					...
					case self::STATUS:	//ステータス
						$item->setIsOpen((int)$values[$this->factors[$i]]);
						break;
					case self::KEYWORD:	//キーワード
						$item->setAttribute("keywords", $values[$this->factors[$i]]);
						break;
					case self::IMG_S:	//一覧画像
						if(!file_exists(SOYSHOP_SITE_DIRECTORY . "files/" . $item->getCode() . "/")) mkdir(SOYSHOP_SITE_DIRECTORY . "files/" . $item->getCode() . "/");
						if(isset($values[$this->factors[$i]])) $item->setAttribute("image_small", "/" . SOYSHOP_ID . "/files/" . $item->getCode() . "/" . $values[$this->factors[$i++]]);
						if(isset($values[$this->factors[$i]])) $item->setAttribute("image_large", "/" . SOYSHOP_ID . "/files/" . $item->getCode() . "/" . $values[$this->factors[$i]]);
						break;
			...
			//すべての確認が終わったら、削除フラグを確認してインサートする
		...
		SOYShop_DataSets::delete("eccube_import.cat_cor_tbl");
		SOYShop_DataSets::delete("eccube_import.cat_par_tbl");
	}
