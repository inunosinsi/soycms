...
class LanguageListComponent extends HTMLList{
	...
	function populateItem($entity, $lang){
		...
		$this->addLink("create_pc_page_link", array(
			"link" => SOY2PageController::createLink("Config.Detail?plugin=util_multi_language&create=" . $this->config[$lang]["prefix"] . "&carrier=pc"),
			"visible" => ($this->displayCreatePageLink($lang))
		));
		
		$this->addLink("create_i_page_link", array(
			"link" => SOY2PageController::createLink("Config.Detail?plugin=util_multi_language&create=" . $this->config[$lang]["prefix"] . "&carrier=i"),
