...
class UserCustomSearchFieldModule extends SOYShopUserCustomfield{
	...
	function clear($app){
		...
		foreach($cnfs as $key => $field){
			$app->clearAttribute(self::_attrKey($key));
		}
	}
	...
	function doPost($param){
		...
		foreach($cnfs as $key => $field){
			...
			$app->setAttribute(self::_attrKey($key), $v);
		}
	}
	...
	function getForm($app, $userId){
		...
		//if(defined("SOYSHOP_ADMIN_PAGE") && SOYSHOP_ADMIN_PAGE){
			...
			foreach($cnfs as $key => $field){
				...
				if(is_null($value)){
					$app = $this->getApp();
					if(!is_null($app)) $value = $app->getAttribute(self::_attrKey($key));
				}
	...
	function buildNamedForm($app, SOYBodyComponentBase $pageObj, $userId=null){
		...
		if(
			...
			foreach($cnfs as $key => $field){
				$usfValue = (isset($app)) ? $app->getAttribute(self::_attrKey($key)) : "";
		...
		//入力画面以外
			...
			foreach(UserCustomSearchFieldUtil::getConfig() as $key => $field){
				$usfValue = (isset($values[$key])) ? $values[$key] : "";
	...
	function confirm($app){
		...
		foreach($cnfs as $key => $field){
			...
			$value = $app->getAttribute(self::_attrKey($key));
	...
	function register($app, $userId){
		...
		//公開側での登録処理
			...
			if(count($cnfs)){
				foreach($cnfs as $key => $field){
					$attrKey = self::_attrKey($key);
					$values[$key] = $app->getAttribute($attrKey);
					$app->clearAttribute($attrKey);
				}
			}
		}
