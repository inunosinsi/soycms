...
class IndexPage extends CMSWebPageBase{
	...
	function doPost(){
		...
		if(soy2_check_token()){
			...
			if(isset($_POST["add"])){
				if(strlen($_POST["Config"]["sign"]) === 0) SOY2PageController::jump("App.Secret?error");
				
				$obj = SOY2::cast("AppDB", $_POST["Config"]);
				
				try{
					$dao->insert($obj);
					SOY2PageController::jump("App.Secret?success");
				}catch(Exception $e){
					//
				}
		...
		SOY2PageController::jump("App.Secret?error");
	}
...
class AccountListComponent extends HTMLList{
	...
	protected function populateItem($entity, $key){
		...
		$this->addActionLink("remove_link", array(
			"link" => SOY2PageController::createLink("App.Secret.Remove." . $entity->getId()),
