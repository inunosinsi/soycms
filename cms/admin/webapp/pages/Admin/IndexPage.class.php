...
class IndexPage extends CMSWebPageBase{
	...
	function __construct($args){
		...
		$this->addLink("reminderconfig", array(
			"link" => SOY2PageController::createLink("Admin.Mail"),
			"visible" => UserInfoUtil::isDefaultUser(),
		));

		$this->addLink("ipconfig", array(
			"link" => SOY2PageController::createLink("Admin.Ip"),
			"visible" => UserInfoUtil::isDefaultUser(),
		));

		$this->addLink("tokenloginconfig", array(
			"link" => SOY2PageController::createLink("Admin.TokenLogin"),
			"visible" => UserInfoUtil::isDefaultUser(),
		));
