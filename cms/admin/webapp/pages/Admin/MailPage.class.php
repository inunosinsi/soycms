...
class MailPage extends WebPage{
	...
	function doPost(){
		...
		if(!soy2_check_token()){
			SOY2PageController::jump("Admin.Mail");
		}
		...
		if(isset($_POST["test_mail_address"])){
			try{
				...
				SOY2PageController::jump("Admin.Mail?sended");
			}catch(Exception $e){
				SOY2PageController::jump("Admin.Mail?failed_to_send");
			}
		...
		//設定更新
			...
			try{
				...
				SOY2PageController::jump("Admin.Mail?updated");
			}catch(Exception $e){
				SOY2PageController::jump("Admin.Mail?failed");
			}
		}
