...
class CalendarExpandSeatOnOutput extends SOYShopSiteOnOutputAction{
	...
	function onOutput($html){
		...
		if(strpos($html, "<!-- output calendar plugin -->")){
			...
			$js = "<script>\n" . file_get_contents(dirname(__FILE__) . "/js/seat.js") . "</script>";
...
SOYShopPlugin::extension("soyshop.site.onoutput", "calendar_expand_seat", "CalendarExpandSeatOnOutput");
