...
//		return $this->getHtml($item->getItemId());
	...
	function getHtml($itemId){
		...
		if(isset($itemId)){	
			$this->prepare();
			$rate = $this->discountLogic->getDiscountRate($this->getItem($itemId));
			if($rate > 0){
	...
	function prepare(){
		...
		if(!$this->discountLogic){
			$this->discountLogic = SOY2Logic::CreateInstance("module.plugins.discount_item_stock.logic.DiscountLogic");
		}
	}
}
