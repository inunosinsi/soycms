...
class ImportCustomerInfoLogic extends ExImportLogicBase{
	...
	private $labels = array("会員ID","お名前(姓)","お名前(名)","お名前(フリガナ・姓)","お名前(フリガナ・名)","郵便番号1","郵便番号2","都道府県","住所1","住所2","E-MAIL","TEL1","TEL2","TEL3","FAX1","FAX2","FAX3","性別","職業","誕生日","備考","登録日","更新日");
			...
			for($i = 1; $i < count($this->factors); $i++){
				...
				switch($i){
					...
					case self::ADRS:		//住所　連続して調べる。ループで戻るよりかマシ
						...
						if(isset($pref) && is_numeric($pref)) $user->setArea($pref);
